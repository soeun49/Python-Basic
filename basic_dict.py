def define_dict():
    ...
    #길이 확인, 포함 여부 확인 가능 --> 대상은 키 목록을 대상으로 한다
    #but, 연결, 반복은 지원하지 않음
    ...
    #키 목록, 값의 목록이 있을때, -> zip 객체 이용
    keys=('one','two','three')
    values=(1,2,3,4)
    d3=dict(zip(keys,values))
    print(d3,type(d3)) #짝이 없는 객체는 출력하지 않는다 - 쌍으로만 출력

    # 사전의 키: 변경 불가 객체만 사용 가능함
    d4={}
    d4[True] = "true"
    d4[10]=10
    d4['eleven'] =11
    ...
    # d4[[1,2,3]]=100 -> unhashable type: List => TypeError
...
def dict_methods():
    ...
    print(lst_keys,type(lst_keys)) #list

    #사전의 값의 목록: values()
    values=dct.values()
    print("값의 목록:",values, type(values)) #dict_values
    #dict_values역시 순차형 -> 다른 순차형으로 변환 가능

    #(키,값) 쌍튜플의 목록: items()
    items = dct.items()
    print("(키,값)의 목록",items,type(items)) #dict_items
    #dict_items= 순차형 -> 다른 순차형으로 변환 가능
...
def using_dict():
    ...
    #키에 접근 vs.get() 메서드
    print("둘리", phones['둘리'])
    # print("홍길동", phones['홍길동']) -> KeyError (없는 키에 접근)
    print("홍길동", phones.get('홍길동')) #.get()메서드는 없는 키에 접근 하면 : None을 반환

    #get 메서드를 사용하는 이유
    #1. 키가 없어도 오류 없이 None 값을 할당 받을 수 있음
    #2.  기본값 기능이 있어서 None 값을 받고 싶지 않을 때
    print ("홍길동", phones.get('홍길동', '전화 없음')) #키가 없으면 기본값을 반환한다
