...
def define_set():
    ...
    print("포함 여부:", 2 in numbers, 2 in odds, 2 in evens)

    #캐스팅: 다른 순차형으로 셋 만들기
    s= "Python Programming" #문자열 안쪽에 모두 몇개의 알파벳이 사용되었는가?
    chars=set(s.upper())
    print(s,chars) #중복은 제거가 되고, 순서가 없이
...
def set_methods():
    ...
    print("짝수집합:", evens)
    # evens.remove(4) #remove -> 없는 요소를 삭제하면 KeyError가 발생함

    #집합 업데이트
    evens.update({2,4,6}) #없는 숫자만 추가해준다
    print("짝수 집합:", evens)
...
def set_oper():
    ...
    print("홀수 집합이 전체집합의 부분집합?", odds.issubset(numbers))
