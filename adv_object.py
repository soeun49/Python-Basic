...
def symbol_table():
    ...
    #객체 내부의 __dict__속성을 확인하면 객체 내부의 심볼 테이블 확인 가능
...
def ref_count():
    ...
    y=x # y에 x의 참조 주소를 복사 (참조 카운트 1UP)
    print("REFCOUNT:",sys.getrefcount(x))
...
def object_id():
    ...
    """
    #불변 자료형: 두 개의 값이 같으면 같은 객체; 같은 주소이다
    i1=10
    i2=int(10)
    print("int:",hex(id(i1)),hex(id(i2)))

    #가변 자료형: 값이 같아도 별개의 객체다
    lst1 = [1,2,3]
    lst2= [1,2,3]
    print("list:",hex(id(lst1)), hex(id(lst2)))

    lst3=lst2 #참조 복사 =객체 주소의 복사; 같은 객체
    print("참조 복사:", hex(id(lst2)),hex(id(lst3)))

    # ==은 동등성의 비교 (같은 값이냐?), is는 동일성의 비교 (같은 객체인가?)
    print("불변객체의 비교:", i1 ==i2, i1 is i2) #값이 같고 (동등), 같은 객체이다 (동일)
    print("가변객체의 비교:", lst1==lst2, lst1 is lst2) #값은 같고 (동등), 다른 객체이다 (동일x)
...
def object_copy():
    ...
    print("x", x)
    print("y", y)
    #다른 객체로 복제가 되었지만 내부 객체는 그대로 ->값이 바뀜
    print("x[0] is y[0]?", x[0]is y[0])

    #deepcopy(깊은 복제)
    #가장 하위의 요소로부터 복제 새 객체로 (재귀적으로) 만들어서 객체를 재구성
    y=copy.deepcopy(x)
    print("x is y?", x is y)
    print("x[0] is y[0]?", x[0] is y[0])
